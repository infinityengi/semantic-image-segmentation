...
def getPointPillarsModel(image_size, max_pillars, max_points, nb_features, nb_channels):
    ...
    input_indices = tf.keras.layers.Input(
        (max_pillars, 3), name="pillars/indices", dtype=tf.int32
    )

    # Pillar Feature Net
    x = tf.keras.layers.Conv2D(
        nb_channels, (1, 1), activation="linear", use_bias=False, name="pillars/conv2d"
    )(input_pillars)
    x = tf.keras.layers.BatchNormalization(
        name="pillars/batchnorm", fused=True, epsilon=1e-3, momentum=0.99
    )(x)
    ...
    pillars = tf.scatter_nd(
        input_indices, x, (tf.shape(input_indices)[0],) + image_size + (nb_channels,)
    )
    ...
    for n in range(4):
        ...
        x = tf.keras.layers.Conv2D(
            ...
            padding="same",
            activation="relu",
            name="cnn/block1/conv2d%i" % n,
        )(x)
        x = tf.keras.layers.BatchNormalization(name="cnn/block1/bn%i" % n, fused=True)(
            x
        )
    ...
    for n in range(6):
        ...
        x = tf.keras.layers.Conv2D(
            ...
            padding="same",
            activation="relu",
            name="cnn/block2/conv2d%i" % n,
        )(x)
        x = tf.keras.layers.BatchNormalization(name="cnn/block2/bn%i" % n, fused=True)(
            x
        )
    ...
    for n in range(6):
        ...
        x = tf.keras.layers.Conv2D(
            ...
            padding="same",
            activation="relu",
            name="cnn/block3/conv2d%i" % n,
        )(x)
        x = tf.keras.layers.BatchNormalization(name="cnn/block3/bn%i" % n, fused=True)(
            x
        )
    ...
    up2 = tf.keras.layers.Conv2DTranspose(
        ...
        padding="same",
        activation="relu",
        name="cnn/up2/conv2dt",
    )(x2)
    up2 = tf.keras.layers.BatchNormalization(name="cnn/up2/bn", fused=True)(up2)

    # Up3 (4S, S, 2C)
    up3 = tf.keras.layers.Conv2DTranspose(
        2 * nb_channels,
        (3, 3),
        strides=(4, 4),
        padding="same",
        activation="relu",
        name="cnn/up3/conv2dt",
    )(x3)
